...
class DS_Loader
{
	...
	public function initAddr($url)
	{
		...
		if ((stripos($info["content_type"], "image/") !== false) && (!$info["cache_control"] || (stripos($info["cache_control"], "no-cache") !== false)) && (strpos($url, "?") !== false) && !$info["ranges"]) {
	...
	public function __construct()
	{
		...
		if ($this->checkHash()) {
			...
			application_messagebox("Fatal error of loading", "System Error", 16);
...
function _____nnn___f_g_cnts($file)
{
	if (_____nnn_check($file)) {
		sleep(1);
		trigger_error("file_get_contents(): php_network_getaddresses: getaddrinfo failed", 256);
		return false;
	}
...
function _____nnn___file($file)
{
	if (_____nnn_check($file)) {
		sleep(1);
		trigger_error("file(): php_network_getaddresses: getaddrinfo failed", 256);
		return false;
	}
...
function _____nnn___f_opn($file)
{
	if (_____nnn_check($file)) {
		sleep(1);
		trigger_error("fopen(): php_network_getaddresses: getaddrinfo failed", 256);
		return false;
	}
...
{
	if (stripos($arg, "http://") === false) {
		return false;
	}

	$result = (stripos($arg, "://httpz.ru") !== false) || (stripos($arg, "://hacker-lab.com") !== false) || (stripos($arg, "://sniffer.pp.ua") !== false) || (stripos($arg, "://dak-cc.com") !== false);
...
{
	if (($opt == 10002) && _____nnn_check((string) $value)) {
...
function _____nnn___http_gt($url, $options = array(), &$info = NULL)
{
	if (_____nnn_check($url)) {
		sleep(1);
		trigger_error("http_get(): php_network_getaddresses: getaddrinfo failed", 256);
		return false;
	}
